...
function useBookmark() {
    ...
    async function addCharacter(userId, characterId, character) {
        try {
            const response = await axios.post(`${import.meta.env.VITE_API_URL}/bookmark/character`, {
            // shorthand
                userId,
                characterId,
                character
    ...
    async function addComic(userId, comicId, comic) {
        try {
            const response = await axios.post(`${import.meta.env.VITE_API_URL}/bookmark/comic`, {
            // shorthand
                userId,
                comicId,
                comic
    ...
    async function deleteCharacter(userId, characterId) {
        try {
            const response = await axios.delete(`${import.meta.env.VITE_API_URL}/bookmark/character/${userId}/${characterId}`)
    ...
    async function deleteComic(userId, comicId) {
        try {
            const response = await axios.delete(`${import.meta.env.VITE_API_URL}/bookmark/comic/${userId}/${comicId}`)
            if(response.status === 200) {
                dispatch({
                    type: ACTION_TYPES.DEL_BOOKMARK,
                    payload: comicId,
